...
function scopeLines(scope: ReportInput["scope"], supplyOnly?: ReportInput["supplyOnly"]): string[] {
  ...
  if (stillUnread.length > 0) {
    ...
    lines.push(
      `⚠️ В ${stillUnread.length === 1 ? "сети" : "сетях"} ${esc(named)} из мостов проверен только Wormhole — его реестр токенов бот читает. Остальные мосты там устроены иначе и пока не поддержаны.`
    );
  }
  ...
  if (unverified.length > 0) {
    ...
    lines.push(
      `ℹ️ Столько токена выпущено в ${unverified.length === 1 ? "сети" : "сетях"} ${described.join(", ")}. ` +
        ...
        "Сколько лежит в остальных, неизвестно: они устроены иначе и бот их пока не читает."
    );
  }
