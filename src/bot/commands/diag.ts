...
export function registerDiagCommand(bot: Telegraf) {
  bot.command("diag", async (ctx: Context) => {
    ...
    } else if (storage.verdict === "new-file") {
      footer += `\n\n⚠️ Файла базы при запуске не было — она создана заново. Так выглядит контейнер без тома: ` +
        `подписки /track и кеши пропадают при каждом обновлении. ` +
        `Лечится томом на <code>/data</code> и переменной <code>DB_PATH=/data/bot.db</code>.`;
    } else {
      footer += `\n\n💾 Файл базы был на месте, но это первый запуск с этой проверкой — ` +
        `переживёт ли база деплой, будет видно после следующего обновления.`;
    }
